##Loading libraries # 2 Breast Cancer cell lines only,  New Dataset (Figure 1d in Original Paper)
library(keras)
library(caret)
library(ggpubr)
...
test= read.csv("breast_cancer_testset_1923.csv",sep=",",header=T,stringsAsFactors = F)

##Extract explanatory variables from test dataset
#xtest = as.matrix(test[,3:1431]) # Orignal

xtest = as.matrix(test[,3:1925])
