##Loading libraries # Old Dataset (Figure 1d in Original Paper)
library(keras)
library(caret)
library(ggpubr)
...
test= read.csv("Test_Set.csv",sep=",",header=T,stringsAsFactors = F) # Orginal
...
# Second input is an index vector (Bootstrapped Index)
fc <- function(d, i){
