##loading libraries Old Dataset (Figure 2a in Original Paper)
library(keras)
library(caret)
library(ggpubr)
...
testData = read.table("test_set1.csv",sep=",",header=T,stringsAsFactors = F,row.names=1)
xtest = as.matrix(testData)

##Reading actual labels
labels = read.table("ic50.csv",sep=",",header=T,stringsAsFactors = F,row.names=1)
